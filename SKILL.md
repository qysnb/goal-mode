...
- **STATE_FILE** = `OPENCODE_GOAL_STATE.json`(同一目录,固定名,用于断点续跑)
...
**检查点 `OPENCODE_GOAL_STATE.json`** — 断点恢复依据,每个里程碑覆盖写:
...
1. 读取项目上下文:**先读 `AGENTS.md`**、目录结构、最近 commit,识别语言/脚本/约定/验证方式(有无测试套件、示例脚本等)
...
- **断点自愈**:若因 LLM 输出上限、网络中断、会话结束等中断,用户再次 `/goal-mode resume` 即可从检查点复原
- **大文件写入**:Write 失败时改用 Bash `cat << 'EOF' > file` 分块写入,不询问
- **文档与注释**:产出代码附必要注释/文档字符串;文档按需更新并记入报告
- **凭据安全**:密钥只经环境变量或询问获得,绝不写入代码或提交仓库
- **环境优先**:动手前先读 `AGENTS.md`/项目约定;无测试套件时用示例脚本/运行命令做验证
